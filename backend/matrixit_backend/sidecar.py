...
def export_excel(
    ...
    # 复用 feishu.py 中的 mapping 构建逻辑（不做网络请求）
...
def export_pdfs(
    ...
    for k, it in items_idx.items():
        ...
        # 使用 os.path.join 或 Path 来正确构建跨平台路径
        if sub_dir:
            # 将 "/" 分隔的路径转换为 Path 对象
            target_dir = out_base
