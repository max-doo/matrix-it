...
- **智能文本高亮**：支持在分析结果（如 TLDR、研究方法）中进行多色高亮与评论标注，高亮内容自动同步至矩阵表格预览。支持文本变更时的智能失效检测。
...
│  │        ├─ TextAnalysisHighlighter.tsx # 文本高亮组件：支持多色标注/评论/Popover交互
...
为支撑大规模文献库（1000+ 条目）的流畅体验，系统进行了深度的全栈优化：

- **渲染性能**：
  - 使用轻量级 `Dropdown` + `Emoji` 替代原生的 `Select` 组件，内存占用降低 60%。
  - 全面优化表格列渲染逻辑，移除所有不必要的闭包与对象创建，实现 60FPS 流畅滚动。
- **进程管理（Rust Monorepo 级优化）**：
  - **F5 刷新保护**：后端实现 Sidecar 单例模式 (`LoadLibraryState`)，在前端刷新或崩溃时自动清理旧进程，彻底解决“僵尸进程”堆积问题。
  - **并发控制**：前端实现毫秒级防抖与并发锁，防止高频操作引发的后端过载。

### 8) 智能文本高亮与评审 (2026.1 新增)

系统内置了轻量级的文本评审系统，支持对 LLM 生成的分析结果进行人工复核与标注：

- **交互设计**：
  - **详情页**：支持划词高亮（4种预设颜色）、添加评论、点击高亮查看/编辑/删除评论。
  - **矩阵表格**：支持高亮内容的**只读预览**，鼠标悬停可查看评论 Tooltip（沉浸式深色主题）。
- **数据持久化**：
  - 高亮数据存储在 `meta_extra.annotations` 中，随 `items` 表持久化到 SQLite。
  - 实现了与 Zotero 同步时的**增量合并策略**，确保重新加载库或刷新 Zotero 数据时，本地的人工标注不会丢失。
- **智能失效**：
  - 当分析字段的文本内容被修改（如重新分析或人工编辑）时，旧的高亮会自动失效（不显示但保留数据），防止位置错乱，确保数据一致性。
